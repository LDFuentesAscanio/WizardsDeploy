...
type ContractedRow = {
  ...
  it_projects: { project_name: string | null } | null;
};
...
export default function CustomerDashboardView() {
  ...
  async function fetchAllData() {
    try {
      ...
      const formattedCategories: FrontendContractedCategoryWithProject[] = (
        contractedRaw ?? []
      ).map((row) => ({
        id: row.id,
  ...
  // 🚩 IMPORTANTE:
  // El modal ya guarda la oferta con la nueva API.
  // Aquí SOLO refrescamos la grilla cuando el modal avisa "listo".
  const handleModalSubmit = async () => {
    try {
      ...
      showError('Failed to refresh offers');
    }
  };
        ...
        <div className="col-span-2 space-y-6">
          ...
          <div className="bg-white/10 backdrop-blur rounded-xl p-6 shadow">
            ...
            {data.categories.length > 0 ? (
              ...
              <p className="text-sm text白/70">No experts requested yet.</p>
            )}
          </div>
        </div>
      </motion.div>
